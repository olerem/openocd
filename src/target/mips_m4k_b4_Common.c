...
int mips_m4k_poll(struct target *target)
{
	...
	if ((ejtag_ctrl & EJTAG_CTRL_ROCC) && (target->state != TARGET_HALTED)) {
		LOG_DEBUG ("unable to clean rocc - erc: 0x%8.8x", ejtag_ctrl);
	}
	...
	if (ejtag_ctrl & EJTAG_CTRL_ROCC) {
		...
		mips_ejtag_set_instr(ejtag_info, EJTAG_INST_CONTROL);
		LOG_DEBUG ("clear rocc: ejtag_ctrl = 0x%8.8x", ejtag_ctrl);
		retval = mips_ejtag_drscan_32(ejtag_info, &ejtag_ctrl);
		if (retval != ERROR_OK) {
		...
		LOG_DEBUG ("read - ejtag_ctrl = 0x%8.8x", ejtag_ctrl);
		mips_ejtag_set_instr(ejtag_info, EJTAG_INST_CONTROL);
		retval = mips_ejtag_drscan_32(ejtag_info, &ejtag_ctrl);
		if (retval != ERROR_OK) {
			LOG_DEBUG ("mips_ejtag_drscan_32 failed: ejtag_ctrl = 0x%8.8x", ejtag_ctrl);
...
int mips_m4k_assert_reset(struct target *target)
{
	...
	if (ejtag_info->ejtag_version != EJTAG_VERSION_20) {
		LOG_DEBUG ("target->reset_halt: %x", target->reset_halt);
		if (target->reset_halt) {
			/* use hardware to catch reset */
			mips_ejtag_set_instr(ejtag_info, EJTAG_INST_EJTAGBOOT);
			LOG_DEBUG("Issued EJTAGBOOT");
		} else {
			mips_ejtag_set_instr(ejtag_info, EJTAG_INST_NORMALBOOT);
			LOG_DEBUG("Issued NORMALBOOT");
		}
	}
...
#if 0
extern uint32_t jtag_tap_idcode (void);
int mips_m4k_assert_reset(struct target *target)
{
	struct mips_m4k_common *mips_m4k = target_to_m4k(target);
	struct mips_ejtag *ejtag_info = &mips_m4k->mips32.ejtag_info;
	...
	LOG_DEBUG("mips_m4k_assert_reset: target->state: %s",
			  target_state_name(target));
...
//	idcode = jtag_tap_idcode ();
	int retval = mips_ejtag_get_idcode(ejtag_info, &idcode);
//	if (retval != ERROR_OK)
//		return retval;
	ejtag_info->idcode = idcode;
	LOG_INFO ("idcode: %x", idcode);
	if (((idcode >> 1) & 0x7FF) == 0x29) {
		LOG_INFO("PIC32MX Detected - using EJTAG Interface");

		/* use microchip specific MTAP reset */
		jtag_add_reset(0, 1);
		mips_ejtag_set_instr(ejtag_info, MTAP_SW_ETAP); /* 0x05 */
		jtag_add_reset(0, 0);
		mips_ejtag_set_instr(ejtag_info, EJTAG_INST_EJTAGBOOT);

//		mips_ejtag_set_instr(ejtag_info, MTAP_SW_MTAP); /* 0x04 */
//		mips_ejtag_set_instr(ejtag_info, MTAP_COMMAND); /* 0x07 */

//		mips_ejtag_drscan_8_out(ejtag_info, MCHP_ASERT_RST); /* 0xD1 */
//		mips_ejtag_drscan_8_out(ejtag_info, MCHP_DE_ASSERT_RST); /* 0xd0 */
...
//		mips_ejtag_set_instr(ejtag_info, MTAP_SW_ETAP); /* 0x05 */
	...
	LOG_INFO ("jtag_reset_config: %x", jtag_reset_config);
//	if (!(jtag_reset_config & RESET_SRST_PULLS_TRST) &&
//			(jtag_reset_config & RESET_SRST_NO_GATING)) {
	if (!(jtag_reset_config & RESET_SRST_PULLS_TRST)){
		LOG_INFO ("!RESET_SRST_PULLS_TRST");
		if (jtag_reset_config & RESET_SRST_NO_GATING) {
			LOG_DEBUG ("issue SRST reset");
			jtag_add_reset(0, 1);
			srst_asserted = true;
		}
	}
	...
	if (ejtag_info->ejtag_version != EJTAG_VERSION_20) {
		...
		if (target->reset_halt) {
			...
			LOG_DEBUG ("NORMALBOOT");
			mips_ejtag_set_instr(ejtag_info, EJTAG_INST_NORMALBOOT);
		}
	}

	if (jtag_reset_config & RESET_HAS_SRST) {
		LOG_INFO ("RESET_HAS_SRST");
		/* here we should issue a srst only, but we may have to assert trst as well */
		if (jtag_reset_config & RESET_SRST_PULLS_TRST){
			LOG_INFO ("SRST & TRST issued");
			jtag_add_reset(1, 1);
		}
		else if (!srst_asserted){
			LOG_INFO("SRST ONLY");
...
//	}
	...
	target->state = TARGET_RESET;
	jtag_add_sleep(50000);
//	LOG_DEBUG ("jtag_add_sleep");
//	jtag_add_sleep(ejtag_info->scan_delay);
...
//	int cur_pc = mips32->core_cache->reg_list[MIPS32_PC].value;
	...
	if (breakpoint->type == BKPT_HARD) {
		...
		if ((breakpoint->length == 4) || ((breakpoint->length == 5) && ((breakpoint->address % 4) == 0))) {
			...
			LOG_DEBUG ("breakpoint->address: %x", breakpoint->address);
			LOG_DEBUG ("(breakpoint->length & 0xE): %x", (breakpoint->length & 0xE));
			...
			LOG_DEBUG ("breakpoint->orig_instr: %x", *breakpoint->orig_instr);
			...
			if ((breakpt_instr == MIPS32_SDBBP) && (verify != MIPS32_SDBBP)) {
				...
				return ERROR_OK;
			} else if ((breakpt_instr == MICRO_MIPS32_SDBBP) && (verify != MICRO_MIPS32_SDBBP)) {
			...
			LOG_DEBUG ("breakpoint->orig_address: %x", breakpoint->address);
			...
			if ((breakpt_instr == MIPS16_SDBBP) && (verify != MIPS16_SDBBP)) {
					LOG_ERROR("Unable to set 16bit breakpoint at address %08" PRIx32
							  " - check that memory is read/writable", breakpoint->address);
					return ERROR_OK;
			} else if ((breakpt_instr == MICRO_MIPS_SDBBP) && (verify != MICRO_MIPS_SDBBP)) {
...
int mips_m4k_read_memory(struct target *target, uint32_t address,
...
int mips_m4k_write_memory(struct target *target, uint32_t address,
...
//atic int mips_m4k_init_target(struct command_context *cmd_ctx,
int mips_m4k_init_target(struct command_context *cmd_ctx,
