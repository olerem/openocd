				   ...
				   int timeout_ms, uint32_t exit_point, struct mips32_common *mips32)
{
	...
	if (retval != ERROR_OK || target->state != TARGET_HALTED) {
		LOG_INFO ("Forcing Halt to error");
...
COMMAND_HANDLER(mips32_handle_ci20_reset_command)
{
	struct target *target = get_current_target(CMD_CTX);
	struct mips32_common *mips32 = target_to_mips32(target);
	struct mips_ejtag *ejtag_info = &mips32->ejtag_info;
	...
	LOG_INFO("Select CI20 Core0...");
	jtag_poll_set_enabled (0);
	jtag_add_reset(0, 1);
	...
	mips_ejtag_set_instr(ejtag_info, 0x15);
	mips_ejtag_set_instr(ejtag_info, 0x0d);

	mips_ejtag_set_instr(ejtag_info, 0x14);
	mips_ejtag_set_instr(ejtag_info, 0x0c);
	mips_ejtag_set_instr(ejtag_info, 0x0c);
	jtag_add_reset(0, 0);

	jtag_poll_set_enabled(1);
...
static const struct command_registration mips32_exec_command_handlers[] = {
	...
	{
		.name = "ci20_reset",
		.handler = mips32_handle_ci20_reset_command,
		.mode = COMMAND_ANY,
		.help = "force ci20 reset",
		.usage = "[value]",
	},
