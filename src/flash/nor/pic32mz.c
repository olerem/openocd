...
#define PIC32MZ_DEVCFG0_1_2	0xBFC00BFC
#define PIC32MZ_DEVCFG0		0xBFC02FFC
#define PIC32MZ_DEVCFG1		0xBFC02FF8
#define PIC32MZ_DEVCFG2		0xBFC02FF4
#define PIC32MZ_DEVCFG3		0xBFC02FF0
#define PIC32MZ_DEVID		0xBF80F220

#define PIC32MZ_BMXPFMSZ	0xBF882060
#define PIC32MZ_BMXBOOTSZ	0xBF882070
#define PIC32MZ_BMXDRMSZ	0xBF882040

/* pic32mz flash controller register locations */

#define PIC32MZ_NVMCON		0xBF80F400
#define PIC32MZ_NVMCONCLR	0xBF80F404
#define PIC32MZ_NVMCONSET	0xBF80F408
#define PIC32MZ_NVMCONINV	0xBF80F40C
...
#define PIC32MZ_NVMKEY		0xBF80F410
#define PIC32MZ_NVMADDR		0xBF80F420
#define PIC32MZ_NVMADDRCLR	0xBF80F424
#define PIC32MZ_NVMADDRSET	0xBF80F428
#define PIC32MZ_NVMADDRINV	0xBF80F42C
#define PIC32MZ_NVMDATA		0xBF80F430
#define PIC32MZ_NVMSRCADDR	0xBF80F440
...
#define MX_1_2			1	/* Pic32mz1xx/2xx */
...
static int pic32mz_protect_check(struct flash_bank *bank)
{
	...
	if (pic32mz_info->dev_type == MX_1_2)
		config0_address = PIC32MZ_DEVCFG0_1_2;
	else
		config0_address = PIC32MZ_DEVCFG0;

	target_read_u32(target, config0_address, &devcfg0);

	if ((devcfg0 & (1 << 28)) == 0) /* code protect bit */
		num_pages = 0xffff;			/* All pages protected */
	else if (Virt2Phys(bank->base) == PIC32MZ_PHYS_BOOT_FLASH) {
		if (devcfg0 & (1 << 24))
		...
		uint32_t offset, uint32_t count)
{
	...
	if (pic32mz_info->dev_type == MX_1_2) {
		/* 128 byte row */
		pic32mz_flash_write_code[8] = 0x2CD30020;
		pic32mz_flash_write_code[14] = 0x24840080;
		pic32mz_flash_write_code[15] = 0x24A50080;
		pic32mz_flash_write_code[17] = 0x24C6FFE0;
		row_size = 128;
	} else {
		/* 512 byte row */
		pic32mz_flash_write_code[8] = 0x2CD30080;
		pic32mz_flash_write_code[14] = 0x24840200;
		pic32mz_flash_write_code[15] = 0x24A50200;
		pic32mz_flash_write_code[17] = 0x24C6FF80;
		row_size = 512;
	}
...
static int pic32mz_write_word(struct flash_bank *bank, uint32_t address, uint32_t word)
{
	...
	target_write_u32(target, PIC32MZ_NVMDATA, word);
...
static int pic32mz_probe(struct flash_bank *bank)
{
	...
	/* Check for Pic32mz1xx/2xx */
	for (i = 0; pic32mz_devs[i].name != NULL; i++) {
		if (pic32mz_devs[i].devid == (device_id & 0x0fffffff)) {
			if ((*(pic32mz_devs[i].name) == '1') || (*(pic32mz_devs[i].name) == '2'))
				pic32mz_info->dev_type = MX_1_2;
			break;
		}
	}
	...
	if (Virt2Phys(bank->base) == PIC32MZ_PHYS_BOOT_FLASH) {
		 ...
		 * fixed 12k */
		if (target_read_u32(target, PIC32MZ_BMXBOOTSZ, &num_pages) != ERROR_OK) {
			LOG_WARNING("PIC32MZ flash size failed, probe inaccurate - assuming 12k flash");
			num_pages = (12 * 1024);
		}
#else
		...
		if (target_read_u32(target, PIC32MZ_BMXPFMSZ, &num_pages) != ERROR_OK) {
			if (pic32mz_info->dev_type == MX_1_2) {
				LOG_WARNING("PIC32MZ flash size failed, probe inaccurate - assuming 32k flash");
				num_pages = (32 * 1024);
			} else {
				LOG_WARNING("PIC32MZ flash size failed, probe inaccurate - assuming 512k flash");
				num_pages = (512 * 1024);
			}
		}
	}
